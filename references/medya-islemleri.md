...
- Kullanıcı tercihlerinde `auto_viewing_breaks:true` varsa yeni videolu işte beta analizini orkestratör bir kez başlatır ve aynı source/subtitle/parametre hash'i olmadan cache'i kullanmaz. Uygun durak bulunamazsa yalnız gerçek görüntü/ses/yakın diyalog incelemesine dayalı `reviewed_no_suitable` kanıtı teslim edilebilir; videosuz iş `no_media`/`not_assessed` kalır. Bu akış altyazı görsel QA'sından ayrıdır.
- `verify_render_qa.py --video film.mkv --ass film.tr.ass --output goruntu/` gerçek FFmpeg/libass PNG örnekleri üretir. `--times 10.4 50.2`, `--fonts <dizin>` desteklenir. Bunu yalnız kullanıcı açıkça görsel PNG/render QA istediğinde çağır. Araç her örnekte input-side seek ve özgün zaman damgalarıyla yalnız hedef çevresini çözer. İstenirse aynı seri/release/profil/typeset düzeninde ilk bölümde en fazla dört amaçlı örnekle görsel temel kurulabilir; bu açık talep kapsamında sonrakilerde aynı imza ve risksiz teknik rapor varsa PNG tekrarı gerekmez, tetikleyici varsa yalnız hedefli PNG açılır. Görsel QA istenmediyse altyazı paketleri ile hedef oynatıcıda erken/orta/geç metin görüntüsüz doğrulanır; teknik ve dil kontrolleri sürer, teslim özeti görsel incelemenin yapılmadığını açıkça belirtir. Devralınmış temel güncel bölümün görüntüsünün incelendiği iddiası değildir. Hedef oynatıcı ve HDR renkleri ayrıca kontrol edilmedikçe doğrulanmış sayılmaz.
...
Kullanıcının kalıcı tercihi nedeniyle `resources/preferences.json` içindeki `auto_content_filter:true` yeni videolu işlerde varsayılan olarak aktiftir. Etkin kapsam `resources/content_filter_profile.json` dosyasındadır. Bu, anahtar kelimeye bakıp otomatik kesim yapmak anlamına gelmez: **her gerçek kesim görüntü/ses ve hikâye bağlamında doğrulanmalıdır.**

Sezon Bağlam Ajanı bütün sezon metnini zaten okurken metinden görülebilen muhtemel sahneleri `content_filter_candidates` olarak işaretler. Bunlar yalnız adaydır; konuşmasız öpüşme veya yakınlaşma sahnesi metinde görünmeyebilir. Her bölümde ayrı İçerik İnceleme Ajanı, `season_context.py derive-filter-review` çıktısını alır, bölümün tamamını görsel olarak tarar, aday bölgeleri daha yoğun inceler ve kesin kesim aralıklarını belirler. Bu ajan bütün sezonu ikinci kez okumaz.

Varsayılan kapsam: açık romantik/cinsel dudaktan öpüşme, cinsel yakınlık/eylem ve romantik-cinsel yatak sahneleri. Sıradan uyuma/dinlenme/hastalık, ailevi-platonik yanak/alın öpücüğü ve yalnız sözlü cinsel gönderme tek başına kesilmez. Belirsiz sahne görsel olarak doğrulanmadan çıkarılmaz.

İçerik İnceleme Ajanı karar ve kanıtı `cuts` listesine yazar; **medyayı kendisi elle kesmez**. Kesimi, altyazı yeniden zamanlamasını, keyframe hizalamasını ve kanıt arşivini `content_filter_pipeline.py` deterministik olarak uygular. Politika kaydı, kullanıcının bu kalıcı tercihine bağlanır. Başlangıç şeması `resources/cut_plan_template.json` içindedir:
...
Önce `content_filter_pipeline.py --video film.mkv --cuts cuts.json` ile planı incele. Motor kesimin başlangıcını önceki, sonunu sonraki doğrulanmış anahtar kareye genişletir; fazladan kesilecek süreyi raporlar. Yakın kareye daraltıp istenmeyen içeriği bırakmaz. Sınırlar belirgin hikâye kaybı oluşturuyorsa planı yeniden değerlendir; kayıtlı profil kapsamı dışında kesim kararı alma. Kesim bir sonraki tutulan sahnenin anlaşılmasını bozuyorsa `context_note` yalnız gerekli hikâye bilgisini kısa ve spoiler eklemeden taşır; gereksiz açıklama yazılmaz.
